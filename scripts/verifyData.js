...
const verifyData = async () => {
  try {
    ...
    // Users
    const users = await User.find({}).select("firstName lastName emailId isPremium skills");
    console.log(`👥 Total Users: ${users.length}`);
    console.log("📝 Sample Users:");
    users.slice(0, 5).forEach(user => {
      console.log(`   • ${user.firstName} ${user.lastName} (${user.emailId}) - ${user.isPremium ? 'Premium' : 'Free'}`);
    });

    // Connection Requests
    const requests = await ConnectionRequest.find({}).populate("fromUserId toUserId", "firstName lastName");
    ...
    requests.slice(0, 3).forEach(req => {
      console.log(`   • ${req.fromUserId.firstName} → ${req.toUserId.firstName} (${req.status})`);
    });
    ...
    chats.slice(0, 3).forEach((chat, index) => {
      console.log(`   ${index + 1}. ${chat.participants[0].firstName} ↔ ${chat.participants[1].firstName} (${chat.messages.length} messages)`);
    });
