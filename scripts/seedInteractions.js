  ...
  try {
    ...
    for (let i = 0; i < Math.min(50, users.length * 2); i++) {
      ...
      const existingRequest = requests.find(
        (req) =>
          (req.fromUserId.toString() === fromUser._id.toString() &&
            req.toUserId.toString() === toUser._id.toString()) ||
          (req.fromUserId.toString() === toUser._id.toString() &&
            req.toUserId.toString() === fromUser._id.toString())
      );
...
const generateChatMessages = async () => {
  try {
    ...
    for (const connection of acceptedConnections.slice(0, 15)) {
      // Limit to 15 chats
